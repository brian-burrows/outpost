...
def process_ingestion_task(task: OwmIngestionTask):
...
def make_upstream_consumer_and_dlq() -> TaskProcessingManager:
    ...
    task_consumer = TaskConsumer(executable = process_ingestion_task)
...
def make_downstream_producer_and_persistent_storage() -> TaskProducerManager:
...
def handle_PEL_batch(producer, consumer):
...
def check_pending_entries_list(
    ...
    try:
        while not catcher.stop_event.is_set():
            start_time = time.time()
            while True:
                num_tasks = handle_PEL_batch(producer, consumer)
                if num_tasks == 0:
                    break
            elapsed = time.time() - start_time
