...
def test_max_fail_on_rate_limit(redis_client):
    ...
    for count, expected in zip(range(3), [True, True, False]):
...
def test_limit_resets_after_day_boundary(redis_client):
    ...
    with patch('owm.limiter.datetime.datetime') as mock_dt:
        ...
        assert limiter.allow_request() == True  # Request 1 (Day 1)
        assert limiter.allow_request() == True  # Request 2 (Day 1)
        assert limiter.allow_request() == False # Request 3 (Blocked)
        ...
        assert limiter.allow_request() == True  # Request 1 (Day 2)